<div class="list-group-item dark:bg-gray-800 {{ $thread->pinned ? 'pinned' : '' }} {{ $thread->locked ? 'locked' : '' }} {{ $thread->trashed() ? 'deleted' : '' }}" :class="{ 'border-primary': selectedThreads.includes({{ $thread->id }}) }">
    <div class="row align-items-center text-center">
        <div id="badgeColor" class="col-sm text-md-start" style="color: {{ $thread->category->color }};">
            <span class="lead">
                <a class="font-bold underline" href="{{ Forum::route('thread.show', $thread) }}" style="color: {{ $thread->category->color }};" @if (isset($category))style="color: {{ $category->color }};" @endif>{{ $thread->title }}</a>
            </span>
            <br>
